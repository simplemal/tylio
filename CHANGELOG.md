...
- **Palette picker now highlights the active preset on page load** (Theme → Presets). Previously the highlight relied on `theme.palette.name` matching the preset id; legacy installs whose seed used `name: 'terra'` (no preset matches by that name) and palettes that were manually edited without renaming both showed as "no preset selected" until the user clicked one. Theme.vue now does a value-by-value structural match at `onMounted` and assigns the detected preset name locally (no DB write — the dirty-flag stays clean until a real change).
