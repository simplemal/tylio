...
const emit = defineEmits<{ close: []; pick: [MediaItem] }>()
...
async function remove(m: MediaItem, e: Event) {
  e.stopPropagation()
  if (
    !(await confirm({
    ...
    >
      ...
      >
        <header class="px-5 py-4 border-b border-white/5 flex items-center gap-3">
          ...
          <h2 id="media-library-title" class="font-display text-xl">{{ t('media.libraryDialogTitle') }}</h2>
          <button type="button" class="btn-icon ml-auto" :aria-label="t('common.close')" @click="emit('close')">
        ...
        <div
          ...
          :class="dragOver ? 'border-ink-100 bg-ink-100/10' : 'border-ink-300/50 hover:border-ink-100'"
